...
export default function PriceCalculatorPage() {
  ...
  if (orderType === 'TITIP_BELI') {
    if (items && items.length > 0) {
      ...
      const singleTransportFeeIdr = roundUpToThousand(TRANSPORT_JPY * EFFECTIVE_RATE);
      ...
      items.forEach((item) => {
        ...
        // Akumulasi Biaya Transport berdasarkan Qty
        transportFeeIdr += singleTransportFeeIdr * qty;
      });

      // 1. Total Harga Barang (Konversi total Yen & Bulatkan)
      totalItemsPriceIdr = roundUpToThousand(totalJpy * EFFECTIVE_RATE);

      // 2. Fee Jastip (Bulatkan Fee Tiering + Gabungkan Total Transport Fee)
